<template>
    <fieldset class="indi-form__fieldset">
        <div class="indi-form__fieldset-title">
            <legend class="indi-form__fieldset-legend">{{title}}</legend>
        </div>
        <div class="indi-form__input-row">

            <div v-for="field in fields" class="indi-form__input-group" v-bind:class="getClass(field.col, field.offset)" >

                <template v-if="!field.field_type || field.field_type === 'input' ">
                    <Input :field="field.field" :label="field.label"
                           :help_txt="field.help_txt" :error_message="field.error_message"
                           :value="data[field.field]" :hidden="field.hidden" :disabled="field.disabled" @on-update="update"/>
                </template>

                <template v-if="field.field_type && field.field_type === 'checkbox' ">
                    <CheckBox :field="field.field" :label="field.label"
                           :help_txt="field.help_txt" :error_message="field.error_message"
                           :value="data[field.field]" @on-update="update" :hidden="field.hidden" :disabled="field.disabled"/>
                </template>

                <template v-if="field.field_type && field.field_type === 'radio' ">
                    <RadioGroup :field="field.field" :label="field.label"
                                :value="data[field.field]" :options="field.options" :stacked="field.stacked"
                                :hidden="field.hidden" @on-update="update" :disabled="field.disabled"/>
                </template>

                <template v-if="field.field_type && field.field_type === 'select' ">
                    <Select :field="field.field" :label="field.label"
                                :value="data[field.field]" :options="field.options" :hidden="field.hidden" :disabled="field.disabled"
                            @on-update="update"/>
                </template>


                <template v-if="field.field_type === 'button' ">
                    <ButtonComponent :label="field.label" :type="field.type" @on-update="update" :field_name="field.field"
                                     :hidden="field.hidden">
...
<script>
    ...
    export default {
        name: "FieldSet",
        components: {ButtonComponent, Input, CheckBox, RadioGroup, Select},
        props:['title', 'fields', 'data'],
        methods:{
            getClass(col, offset){
                let classValue = "indi-form__input-group--" + col;
                if(offset)
                    classValue+= " indi-form__input-group--offset-"+offset;
                return classValue;
            },
            update(field){
                this.data[field.field_name] = field.value;
                this.$emit('on-update',field);
            }
        }
    }
</script>
