...
<script>
    /**
     * Base field has all common properties and methods.
     */
    export default {
        name: "BaseField",
        props: ['error_message', 'help_txt', 'value', 'field', 'label', 'disabled', 'hidden'],
        methods:{
            update(field_name, event){
                this.$emit('on-update', { field_name, value : event.target.value });
            },
            getClass(){
                if(this.disabled){
                    return ' indi-form__input--disabled';
                }else{
                    return '';
                }
            }
