<template>
    <div v-if="hidden !== true">
        <label class="indi-form__checkbox" >
            <input type="checkbox" class="indi-form__checkbox-input" @click="checked($event)" v-model="checkLocal" v-bind:class="getClass()"/>
            <span class="indi-form__checkbox-icon"></span>
            <span class="indi-form__checkbox-text"> {{ label }} </span>
        </label>
    </div>
...
<script>
    ...
    export default {
        name: "CheckBox",
        extends: BaseField,
        data:()=> ({checkLocal: false}),
        methods:{
            checked(e){
                this.$emit('on-update', { field_name:this.field, value : e.target.checked });

            }
        },
        created(){
            this.checkLocal = this.value;
        }
