<template>
    <div v-if="hidden !== true" >
        <label v-if="label" class="indi-form__label">{{label}}</label>
        <input class="indi-form__input" type="text" :value=value @input="update(field,$event)" v-bind:class="getClass()">
        <div v-if="help_txt" class="indi-form__help-text"> {{ help_txt }} </div>
        <div v-if="error_message" class="indi-form__input-notification indi-form__input-notification--has-error"> {{ error_message }} </div>
    </div>
</template>
